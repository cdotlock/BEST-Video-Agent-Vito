...
1. 项目页主舞台顶部新增 `灵动岛（Dynamic Island）`，默认是单条动态状态岛，显式展示当前阶段、当前 cue、关键阻塞与下一动作
2. `灵动岛` 展开后同时承载运行监控、导演编排与 `Pro` 叠层，不再把 `Pro` 作为独立重复入口悬挂在顶栏
3. `Pro` 的 `Strategy` 层把分镜密度、参考路线、角色/空镜/对白/粗剪策略转成导演蓝图，并把 review gate 收口到同层
4. `Capabilities + 灵动岛 + Strategy` 三层共同承担“高上限但不暴露源码”的交互边界
5. 项目页顶部标题栏不再重复展示项目标题、序列与执行模式摘要；聊天历史改为主舞台内左侧会话列表承载
